...
const DEFAULT_POSITION = "Social Media Manager (Sports Focus)";
...
function logApplicationIssue(label, error) {
  console.error(`[social-media-manager-apply] ${label}`, {
...
function getApplicationErrorUrl(statusCode, message) {
  const url = new URL("https://jcit.digital/smm/");
...
function buildApplicantEmail({ name, position }) {
  ...
  const subject = `Next step for your JCIT Services application`;

  const text = [
    `Hi ${safeName}!`,
    ``,
    `Thank you for applying for the ${safePosition} role at JCIT Services. We have received your application and would like to move forward with the next step.`,
    ``,
  ...
  const html = `
    <div style="margin:0;padding:32px 0;background:#06121d;background-image:radial-gradient(circle at 85% 15%, rgba(102,255,204,0.15), transparent 30%),radial-gradient(circle at 10% 85%, rgba(0,240,255,0.15), transparent 30%);font-family:Arial,sans-serif;color:#e8fbff;">
      <div style="max-width:680px;margin:0 auto;padding:0 20px;">
        <div style="padding:28px;border-radius:28px;background:linear-gradient(180deg, rgba(11,26,41,0.96), rgba(7,18,29,0.98));border:1px solid rgba(183,241,255,0.14);box-shadow:0 24px 60px rgba(0,0,0,0.38);">
          <p style="margin:0 0 8px;font-size:12px;letter-spacing:0.18em;text-transform:uppercase;color:#9ec2d5;">JCIT Services</p>
          ...
          <p style="margin:0 0 16px;font-size:16px;line-height:1.75;color:#c9e7f2;">Thank you for applying for the <strong>${escapeHtml(safePosition)}</strong> role at JCIT Services. We have received your application and would like to move forward with the next step.</p>
          ...
          <p style="margin:0;font-size:15px;line-height:1.7;color:#f4fdff;">Best regards,<br /><strong>JCIT SERVICES</strong></p>
        </div>
      </div>
    </div>
  `.trim();
...
function buildInternalEmail({ fields, resumeFile }) {
  ...
  const rows = [
    ...
    ["Position", position],
    ["Portfolio / Managed Account", fields.portfolio_url],
    ["Editing / Scheduling Tools", fields.tools_experience],
    ["Sports Experience", fields.sports_experience],
    ["Role Acknowledgement", fields.role_acknowledgement],
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;color:#102331;">
      <h1 style="margin:0 0 12px;">New JCIT Services application</h1>
      <p style="margin:0 0 18px;">A new applicant submitted the ${escapeHtml(position)} form.</p>
...
async function forwardToFormspree({ fields, resumeFile }) {
  ...
  const forwardedFields = {
    _subject: `JCIT Services - ${toSafeLine(fields.position) || DEFAULT_POSITION} Application`,
    ...
    tools_experience: fields.tools_experience,
    sports_experience: fields.sports_experience,
...
async function handleApplication(req, res) {
  ...
  const position = toSafeLine(fields.position) || DEFAULT_POSITION;
  const portfolioUrl = toSafeLine(fields.portfolio_url);
  const toolsExperience = toSafeLine(fields.tools_experience);
  const sportsExperience = toSafeLine(fields.sports_experience);
  ...
  if (!portfolioUrl) {
    const error = new Error("Portfolio or managed account link is required.");
    error.statusCode = 400;
    throw error;
  }
  if (!toolsExperience) {
    const error = new Error("Editing and scheduling tools experience is required.");
    error.statusCode = 400;
    throw error;
  }
  if (!sportsExperience) {
    const error = new Error("Sports experience is required.");
    error.statusCode = 400;
    throw error;
  }
