...
const SOURCE_LABEL = "jcit.digital/status (fault-report)";
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const message =
      statusCode >= 500
        ? "Report could not be submitted right now. Please try again later."
        : error.message;
    if (wantsJson(req)) return sendJson(res, statusCode, { ok: false, message });
    redirect(res, `https://jcit.digital/status/?fault=error&message=${encodeURIComponent(message)}`);
  }
};
