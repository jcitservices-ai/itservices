...
function getApplicationErrorUrl(statusCode, message) {
  const url = new URL(APPLICATION_PAGE_URL);
...
function buildApplicantEmail({ name, position }) {
  ...
  const text = [
    `Hi ${safeName}!`,
    "",
    `Thank you for applying for the ${safePosition} role at JC IT Services. We have received your application and your uploaded CV/resume.`,
    "",
    ...
    "What to Expect:",
    "- Quick introduction",
    "- Discussion about your sales experience and outreach approach",
    "- Overview of the SDR role and expectations",
    ...
    <div style="margin:0;padding:32px 0;background:#06121d;background-image:radial-gradient(circle at 85% 15%, rgba(102,255,204,0.15), transparent 30%),radial-gradient(circle at 10% 85%, rgba(0,240,255,0.15), transparent 30%);font-family:Sora,Arial,sans-serif;color:#e8fbff;">
      <div style="max-width:680px;margin:0 auto;padding:0 20px;">
        <div style="padding:28px;border-radius:28px;background:linear-gradient(180deg, rgba(11,26,41,0.96), rgba(7,18,29,0.98));border:1px solid rgba(183,241,255,0.14);box-shadow:0 24px 60px rgba(0,0,0,0.38);">
          ...
          <p style="margin:0 0 16px;font-size:16px;line-height:1.75;color:#c9e7f2;">Thank you for applying for the <strong>${escapeHtml(safePosition)}</strong> role at JC IT Services. We have received your application and your uploaded CV/resume.</p>
          ...
          <div style="margin:0 0 20px;padding:18px;border-radius:20px;background:rgba(255,255,255,0.045);border:1px solid rgba(183,241,255,0.1);">
            ...
            <ul style="margin:0;padding-left:20px;color:#c9e7f2;font-size:15px;line-height:1.8;">
              ...
              <li>Discussion about your sales experience and outreach approach</li>
              <li>Overview of the SDR role and expectations</li>
...
function buildInternalEmail({ fields, resumeFile }) {
  ...
  const rows = [
    ...
    ["Position", position],
    ["Sales Experience", fields.sales_experience],
    ["Primary Channel", fields.primary_channel],
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;color:#102331;">
      ...
      <p style="margin:0 0 18px;">A new applicant submitted the Sales Development Representative form.</p>
...
async function forwardToFormspree({ fields, resumeFile }) {
  ...
  const forwardedFields = {
    ...
    sales_experience: fields.sales_experience,
    primary_channel: fields.primary_channel,
...
async function handleApplication(req, res) {
  ...
  if (!toSafeLine(fields.contact_number)) {
    const error = new Error("Contact number is required.");
    error.statusCode = 400;
    throw error;
  }
  if (!toSafeLine(fields.location)) {
    const error = new Error("Location is required.");
    error.statusCode = 400;
    throw error;
  }
  if (!toSafeLine(fields.sales_experience)) {
    const error = new Error("Sales experience is required.");
    error.statusCode = 400;
    throw error;
  }
  if (!toSafeLine(fields.message)) {
    const error = new Error("Application message is required.");
    error.statusCode = 400;
    throw error;
  }
  ...
  const normalizedFields = { ...fields, email, position };
  const applicantEmail = buildApplicantEmail({ name, position });
  const internalEmail = buildInternalEmail({ fields: normalizedFields, resumeFile });
  ...
  if (notifyEmail) {
    const notifications = [
      ...
      },
      {
        ...
        promise: forwardToFormspree({ fields: normalizedFields, resumeFile }),
      },
    ];
    ...
    await forwardToFormspree({ fields: normalizedFields, resumeFile }).catch((error) => {
