const SOURCE_LABEL = "jcit.digital/dashboard (kickoff)";
const PROJECT_NAME = "Lead Tracker Dashboard";
const CLIENT_COMPANY = "Craftsmanship Marketing";
const KICKOFF_URL = "https://calendar.app.google/cJAkWtezoTPHwK8t5";
const DEFAULT_FROM = "JC IT Services <info@jcit.digital>";
const DEFAULT_REPLY_TO = "info@jcit.digital";
const DEFAULT_NOTIFY_EMAIL = "jake@jcit.digital.com";
...
function logKickoffIssue(label, error) {
  console.error(`[dashboard-kickoff] ${label}`, {
...
function getErrorUrl(message) {
  const url = new URL("https://jcit.digital/dashboard/");
...
function buildClientEmail({ clientName, contactNumber, email }) {
  ...
  const text = [
    ...
    ``,
    `Project summary:`,
    `- Total project value: $500 USD`,
    `- Sprint duration: 10 days`,
    `- Kickoff calendar: ${KICKOFF_URL}`,
    ``,
    `Next step: please book the kickoff meeting using the calendar link above so we can lock scope, confirm communication channels, and start environment setup.`,
    ``,
    `Submission details:`,
    `- Client: ${safeName}`,
    ...
    `- Email: ${safeEmail}`,
    ``,
    `If you need anything before kickoff, reply to this email and our team will help right away.`,
    ``,
    `Best regards,`,
    `JC IT Services`,
  ].join("\n");
        ...
        <div style="padding:28px;border-radius:28px;background:linear-gradient(180deg, rgba(11,26,41,0.96), rgba(7,18,29,0.98));border:1px solid rgba(183,241,255,0.14);box-shadow:0 24px 60px rgba(0,0,0,0.38);">
          ...
          <div style="margin:0 0 18px;padding:18px;border-radius:20px;background:rgba(255,255,255,0.045);border:1px solid rgba(183,241,255,0.1);">
            ...
            <ul style="margin:0;padding-left:20px;color:#c9e7f2;font-size:15px;line-height:1.8;">
              <li>Total project value: $500 USD</li>
              <li>Sprint duration: 10 days</li>
              <li>Kickoff booking: <a href="${KICKOFF_URL}" style="color:#66ffcc;">${KICKOFF_URL}</a></li>
            </ul>
          </div>
          <p style="margin:0 0 16px;font-size:16px;line-height:1.75;color:#c9e7f2;">Next step: please book the kickoff meeting so we can lock scope, confirm communication channels, and start environment setup.</p>
          ...
          <div style="margin:0 0 18px;padding:16px 18px;border-radius:18px;background:rgba(102,255,204,0.08);border:1px solid rgba(102,255,204,0.2);">
            ...
            <p style="margin:0;font-size:15px;line-height:1.7;color:#e8fbff;">Client: ${escapeHtml(safeName)}<br />Contact number: ${escapeHtml(safeContact)}<br />Email: ${escapeHtml(safeEmail)}</p>
          </div>
...
function buildInternalEmail({ clientName, contactNumber, email, approval, source }) {
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;color:#102331;">
      ...
      <p style="margin:0 0 18px;">A client approved the Lead Tracker Dashboard proposal and is ready to schedule kickoff.</p>
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const clientEmail = buildClientEmail({ clientName, contactNumber, email });
