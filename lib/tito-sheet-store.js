...
function getServiceAccount({ required = true } = {}) {
  let parsed = {};
...
async function getAccessToken() {
  const serviceAccount = getServiceAccount({ required: false });
  const authSource = serviceAccount ? "service_account" : "oauth";

  if (
    cachedAccessToken &&
    cachedAccessToken.authSource === authSource &&
  ...
  const credentials = serviceAccount || getServiceAccount();
...
function googleApiErrorMessage(data, fallback) {
  ...
  if (/insufficient authentication scopes/i.test(message)) {
    return [
      "Google access is missing the required write scope.",
      "Reauthorize GOOGLE_OAUTH_REFRESH_TOKEN with https://www.googleapis.com/auth/spreadsheets and https://www.googleapis.com/auth/drive.file,",
      "or configure GOOGLE_SERVICE_ACCOUNT_EMAIL and GOOGLE_PRIVATE_KEY for the shared service account.",
    ].join(" ");
  }
  return message;
}
...
async function googleRequest(path, options = {}) {
  ...
  if (!response.ok) {
    const message = googleApiErrorMessage(data, "Google Sheets request failed.");
...
async function googleDriveRequest(url, options = {}) {
  ...
  if (!response.ok) {
    const message = googleApiErrorMessage(data, "Google Drive request failed.");
    const error = new Error(message);
    error.statusCode = response.status === 403 ? 503 : 502;
    throw error;
  }
...
function rowToCoaching(row) {
  ...
  const project = valueFor(row, CLIENT_PROJECT_ALIASES) || valueFor(row, ASSIGNED_PROJECT_ALIASES);
...
async function acknowledgeCoaching({ username, email, password, coachingId, rowNumber }) {
  ...
  const emailStatus = await sendCoachingAcknowledgementNotifications({
    workbook,
    member,
    coaching,
  });
  const message = emailStatus.failed
    ? "Coaching acknowledged. Some notifications could not be sent."
    : emailStatus.sentToEmployee || emailStatus.sentToCoach
      ? "Coaching acknowledged and notifications sent."
      : "Coaching acknowledged. Notification recipients could not be resolved.";
...
function resolveClockRecipients(workbook, member, projectName) {
  ...
  return {
    employee: {
      label: member.name || member.teamMember || "Team Member",
      emails: uniqueEmails(extractEmails(member.email || member.username)),
    },
    client: {
...
function resolveCoachingAcknowledgementRecipients(workbook, member, coaching) {
  const coachEmails = uniqueEmails(
    resolvePersonEmails(
      workbook,
      coaching.coachEmail || member.coachEmail,
      coaching.coach || member.coach
    )
  );
  const coachFallback = emailsFromEnv("JCIT_TITO_DEFAULT_COACH_EMAILS", DEFAULT_COACH_EMAILS);
  ...
  return {
    ...
    coach: {
      label: coaching.coach || member.coach || "Coach",
      emails: coachEmails.length ? coachEmails : coachFallback,
    },
  };
}
...
async function clockAction({ username, email, password, action, project, activity, notes }) {
  ...
  let projectName = normalizeValue(project);
  let activityName = normalizeValue(activity) || "Client Work";
  ...
  if (normalizedAction === "time_out" && activeShift) {
    projectName = projectName || activeShift.project || "";
    activityName = normalizeValue(activity) || activeShift.activity || "Client Work";
  }
  ...
  if (normalizedAction === "time_in") {
    ...
    const emailStatus = await sendClockNotifications({
      workbook,
    ...
    const notificationSent = emailStatus.sentToEmployee || emailStatus.sentToClient || emailStatus.sentToCoach;
...
function clockEmailContent({
  ...
  const isTimeOut = clockAction === "time_out";
  const eventLabel = isTimeOut ? "time-out" : "time-in";
  const subject = isEmployee
    ? `JCIT TiTo ${eventLabel} saved: ${projectName}`
    : `JCIT TiTo ${eventLabel}: ${member.name} - ${projectName}`;
  const intro = isEmployee
    ? `Your TiTo ${eventLabel} has been saved.`
    : `${member.name} has ${isTimeOut ? "ended" : "started"} a TiTo session.`;
  const rows = [
    ...
    ...(isTimeOut ? [["Time Out", formatEmailDateTime(timeOut)], ["Total Hours", totalHours || "-"]] : []),
    ["Log ID", entryId],
    ["Notes", notes || "-"],
  ];
  const text = [
...
async function sendClockNotification({ role, recipients, content }) {
...
async function sendClockNotifications({
  workbook,
  member,
  projectName,
  activityName,
  ...
  const recipients = resolveClockRecipients(workbook, member, projectName);
  const results = await Promise.all(
    ["employee", "client", "coach"].map((role) =>
      sendClockNotification({
        role,
        recipients: recipients[role].emails,
        content: clockEmailContent({
          audience: role,
          clockAction,
          member,
          projectName,
  ...
  return {
    sentToEmployee: Boolean(byRole.employee?.sent),
    ...
    recipientCounts: {
      employee: byRole.employee?.count || 0,
...
function coachingAcknowledgementEmailContent({ audience, member, coaching, projectName, recipientLabel }) {
  const isEmployee = audience === "employee";
  const memberName = member.name || member.teamMember || member.email || "Team member";
  const subjectProject = projectName ? ` - ${projectName}` : "";
  const subject = isEmployee
    ? `JCIT coaching acknowledged${subjectProject}`
    : `JCIT TiTo coaching acknowledged: ${memberName}${subjectProject}`;
  const intro = isEmployee
    ? "Your JCIT coaching acknowledgement has been recorded."
    : `${memberName} has acknowledged a coaching record in TiTo.`;
  const rows = [
    ["Team Member", memberName],
    ["Email", member.email || "-"],
    ["Project", projectName || coaching.project || "-"],
    ["Topic", coaching.topic || "Coaching Record"],
    ["Type", coaching.type || "-"],
    ["Coach", coaching.coach || member.coach || "-"],
    ["Acknowledged At", formatEmailDateTime(coaching.acknowledgedAt)],
    ["Record ID", coaching.id || `row-${coaching.rowNumber}`],
  ];
  const detailRows = [
    ["Notes", coaching.notes || "-"],
    ["Action Plan", coaching.actionPlan || "-"],
  ];
...
async function sendCoachingAcknowledgementNotification({ role, recipients, content }) {
  if (!recipients.length) {
    return { sent: false, skipped: true, failed: false, count: 0, role, reason: `No ${role} recipient found.` };
  }
...
async function sendCoachingAcknowledgementNotifications({ workbook, member, coaching }) {
  const recipients = resolveCoachingAcknowledgementRecipients(workbook, member, coaching);
  const results = await Promise.all(
    ["employee", "coach"].map((role) =>
      sendCoachingAcknowledgementNotification({
        role,
        recipients: recipients[role].emails,
        content: coachingAcknowledgementEmailContent({
          audience: role,
          member,
          coaching,
  ...
  return {
    ...
    reasons: Object.fromEntries(results.map((result) => [result.role, result.reason || ""])),
  };
}
