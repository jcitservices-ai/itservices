...
async function acknowledgeCoaching({ username, email, password, coachingId, rowNumber }) {
  ...
  const message = emailStatus.failed
    ...
    : emailStatus.sentToClient || emailStatus.sentToCoach
...
function resolveCoachingAcknowledgementRecipients(workbook, member, coaching) {
  ...
  const fallbackProjects = assignedProjects.length === 1 ? assignedProjects : [];
  const projectCandidates = uniqueTextValues([coaching.project, ...fallbackProjects]);
  const client = projectCandidates
    .map((projectName) => findClientForProject(workbook, projectName))
    .find((match) => match?.emails?.length);
  ...
  return {
    client: {
      label: client?.clientName || client?.company || "Client",
      project: client?.project || projectCandidates[0] || coaching.project || "",
      emails: client?.emails || [],
    },
    coach: {
...
function coachingAcknowledgementEmailContent({ audience, member, coaching, projectName, recipientLabel }) {
  ...
  const subject = isClient
    ? `JCIT coaching acknowledged: ${memberName}${subjectProject}`
    : `JCIT TiTo coaching acknowledged: ${memberName}${subjectProject}`;
  const intro = isClient
    ? `${memberName} has acknowledged a JCIT coaching record for the assigned project.`
    ...
    ["client", "coach"].map((role) =>
      sendCoachingAcknowledgementNotification({
        ...
        content: coachingAcknowledgementEmailContent({
          ...
          member,
          coaching,
          projectName: recipients.client.project || coaching.project,
          recipientLabel: recipients[role].label,
        }),
      })
    )
